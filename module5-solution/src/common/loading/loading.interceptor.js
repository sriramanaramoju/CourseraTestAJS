  ...
  return {
    request: function (config) {
     console.log("Inside interceptor, config: ", config);
      if (++loadingCount === 1) {
        console.log("inside LoadingHttpInterceptor request true")
    ...
    response: function (response) {
      if (--loadingCount === 0) {
        console.log("inside LoadingHttpInterceptor response false")
    ...
    responseError: function (response) {
      if (--loadingCount === 0) {
        console.log("inside LoadingHttpInterceptor responseError false")
        $rootScope.$broadcast(loadingEventName, {on: false});
      }
